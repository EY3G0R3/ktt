...
- A worktree agent's reported phase (`workmux_phase`) leads the bottom row on
  ...
  pipeline from `building` to `ready to merge` also draws a six-cell track,
  `■■■□□□` for fixing review by default, pinned to the card's right edge: done
  cells in the phase color, the rest in meta gray, both passed through the
  shared accent helper so they stay visible on the active card's light slate.
  The right edge is the one column every card shares regardless of tree
  depth, so the tracks line up and the bar reads as a progress column. The
  track claims its width before the centered label. Terminal and
  off-pipeline phases such as `needs human design` draw no track. Building is
